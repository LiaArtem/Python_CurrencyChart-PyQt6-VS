    ...
    try:
        ...
        with get_con(data_set_find) as con:        
            ...
            for mas in data_db:
                ...
                cursor.execute("EXECUTE PROCEDURE " + data_set_find[7] + "('" + params[0] + "','" + params[1] + "',"+ str(params[2]) + ");")
            con.commit()
...
def load_data_report(data_set_find = (), type_db = ''):
    ...
    try:
        ...
        with get_con(data_set_find) as con:        
            cursor = con.cursor()
            cursor.execute("select CURS_DATE, CURR_CODE, CAST(RATE AS VARCHAR(10)) AS RATE, CAST(AVG_RATE AS VARCHAR(20)) AS AVG_RATE FROM " + data_set_find[8])
