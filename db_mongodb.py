...
def get_con(data_set_find = ()):                
    myclient = dr.MongoClient("mongodb://" + data_set_find[1] + ":" + 
                              str(data_set_find[2]) + "/", 
    ...
    try:
        ...
        with get_con(data_set_find) as myclient:                    
            ...
            for mas in data_db:                  
                ...
                mydict = { "curr_code" : curr_code,  "curs_date" : my_dt, 
                          "rate" : mas[1], "forc" : 1}
                is_exists = False
...
def load_data_report(data_set_find = (), type_db = ''):
    ...
    try:
        ...
        with get_con(data_set_find) as myclient:                     
            ...
            myquery = {"curr_code": "USD", "curs_date": 
                       {"$lt": end_dt, "$gte": start_dt}}
            mydoc = mycol.find(myquery)
            for mas in mydoc:                
                data_report.append([mas["curs_date"].strftime("%d.%m.%Y"), 
                                    mas["curr_code"], mas["rate"], mas["forc"]])
