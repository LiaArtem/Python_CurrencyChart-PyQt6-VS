...
def get_con(data_set_find = ()):            
    auth_provider = dr.PlainTextAuthProvider(username=data_set_find[4], 
                                             password=data_set_find[5])
    cluster = dr.Cluster([data_set_find[1]], port=data_set_find[2], 
        ...
        with get_con(data_set_find) as session:                    
            ...
            for mas in data_db:
                ...
                results = session.execute(
                    "SELECT COUNT(*) AS KOL FROM " + table_name + 
                    " WHERE curr_code = %s AND curs_date = %s allow filtering", 
                                          params)
                row = results[0]
                if row.kol == 0:
...
def load_data_report(data_set_find = (), type_db = ''):
    ...
    try:
        ...
        with get_con(data_set_find) as session:                     
            table_name = data_set_find[3]  + "." + data_set_find[8]
            results = session.execute(
                "SELECT CAST(curs_date AS text) as curs_date, curr_code, rate, forc " + 
                "FROM " + table_name + 
                " WHERE curr_code = 'USD' allow filtering")       
            data = []
            for mas in results:
